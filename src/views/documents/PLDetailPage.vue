...
function handlePdfDownload() {
  if (!detail.value) return
  const opened = openDocumentOutputByType('PL', detail.value, false)
  if (opened) {
      ...
      <div class="space-y-4 lg:col-span-2">
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            <div><span class="text-slate-500">PL 번호</span><div class="mt-0.5 font-medium">{{ detail.id }}</div></div>
            ...
            <table class="w-full min-w-[760px] text-sm">
              ...
              <tbody class="divide-y">
                <tr v-for="item in detail.items" :key="`${detail.id}-${item.name}`">
                  ...
                  <td class="p-3 text-right">{{ item.netWeight }}</td>
                  <td class="p-3 text-right font-semibold">{{ item.grossWeight }}</td>
