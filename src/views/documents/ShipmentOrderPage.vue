...
import { SHIPMENT_STATUS_LABEL } from '@/utils/enumLabels'
...
const statusOptions = computed(() => buildSelectOptionsFromRows(rowsData.value, 'status', SHIPMENT_STATUS_LABEL))
...
const previewFields = computed(() => {
  ...
  return [
    { label: '출하지시일', value: previewTarget.value.issueDate },
...
function downloadPdf(row) {
  const opened = openDocumentOutputByType('SHIPMENT', row, false)
  if (opened) {
...
<template>
  <div class="fade-in space-y-5">
    <PageHeader title="출하지시서 관리" icon-class="fas fa-truck-loading">
      <template #actions>
        <BaseButton variant="secondary" @click="currentOutputTarget ? downloadPdf(currentOutputTarget) : toast.info('출력할 출하지시서가 없습니다.', 'PDF')">
          <template #leading>
