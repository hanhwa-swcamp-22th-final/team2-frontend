...
<template>
  <div class="pl-page">
    ...
    <table class="items-table">
      ...
      <tbody>
        <tr v-for="(item, index) in packingItems" :key="`${item.name}-${index}`" class="item-row">
          ...
          <td class="text-right accent-blue">{{ item.netWeight }}</td>
          <td class="text-right accent-blue">{{ item.grossWeight }}</td>
          <td class="text-right accent-blue">{{ item.measurement }}</td>
        </tr>
        <tr v-if="itemCount === 0" class="no-items-row">
          <td colspan="5" class="no-items-cell">No items</td>
        </tr>
...
.total-row {
  display: grid;
  grid-template-columns: 1.6fr 0.8fr 0.9fr 0.9fr 0.9fr;
  align-items: center;
  border-top: 2px solid #000;
